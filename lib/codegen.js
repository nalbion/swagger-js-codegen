...
var getViewForSwagger2 = function(opts, type){
    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, m){
            var M = m.toUpperCase();
            if(M === '' || authorizedMethods.indexOf(M) === -1 || M === 'OPTIONS') {
                return;
            }
            var secureTypes = [];
                ...
                if(swagger.securityDefinitions) {
                    for(var sk in swagger.securityDefinitions) {
                        if(mergedSecurity.join(',').indexOf(sk) !== -1){
                            var def = swagger.securityDefinitions[sk],
                                type = def.type;
                            // API Gateway require type to be set to 'apiKey', even if OAuth2 is used.
                            if (def['x-amazon-apigateway-authorizer']) {
                                var exp = def['x-amazon-apigateway-authorizer'].identityValidationExpression;
                                if (exp && exp.match(/^\^?\(?Bearer/)) {
                                    type = 'oauth2';
                                }
                            }
                            secureTypes.push(type);
                        }
                    }
                }
            }
